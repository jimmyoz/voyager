...
func (c *command) setAllFlags(cmd *cobra.Command) {
	cmd.Flags().String(optionNameDataDir, filepath.Join(c.homeDir, ".voyager"), "data directory")
	cmd.Flags().Uint64(optionNameDBCapacity, 5000000, fmt.Sprintf("db capacity in chunks, multiply by %d to get approximate capacity in bytes", infinity.ChunkSize))
	...
	cmd.Flags().StringSlice(optionNameBootnodes, []string{"/dnsaddr/bootnode.ethinfinity.org"}, "initial nodes to connect to")
	...
	cmd.Flags().Uint64(optionNameNetworkID, 1, "ID of the Infinity network")
	...
	cmd.Flags().String(optionNameTracingServiceName, "voyager", "service name identifier for tracing")
	cmd.Flags().String(optionNameVerbosity, "info", "log verbosity level 0=silent, 1=error, 2=warn, 3=info, 4=debug, 5=trace")
	cmd.Flags().String(optionWelcomeMessage, "", "send a welcome message string during handshakes")
	cmd.Flags().Bool(optionNameGlobalPinningEnabled, false, "enable global pinning")
	cmd.Flags().String(optionNamePaymentThreshold, "10000000000000", "threshold in IFI where you expect to get paid from your peers")
	cmd.Flags().String(optionNamePaymentTolerance, "50000000000000", "excess debt above payment threshold in IFI where you disconnect from your peer")
	cmd.Flags().String(optionNamePaymentEarly, "1000000000000", "amount in IFI below the peers payment threshold when we initiate settlement")
