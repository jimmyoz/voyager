...
import (
	...
	"github.com/yanhuangpai/voyager/pkg/settlement/swap/cpuaward"
...
func NewVoyager(addr string, infinityAddress infinity.Address, publicKey ecdsa.PublicKey, signer crypto.Signer, networkID uint64, logger logging.Logger, libp2pPrivateKey, pssPrivateKey *ecdsa.PrivateKey, o Options) (b *Voyager, cpuawardService cpuaward.Service, err error) {
	...
	if o.DebugAPIAddr != "" {
		...
		debugAPIService = debugapi.New(infinityAddress, publicKey, pssPrivateKey.PublicKey, overlayEthAddress, logger, tracer, o.CORSAllowedOrigins)
	...
	kad := kademlia.New(infinityAddress, addressbook, hive, p2ps, logger, kademlia.Options{Bootnodes: bootnodes, StandaloneMode: o.Standalone, BootnodeMode: o.BootnodeMode})
...
func (b *Voyager) Shutdown(ctx context.Context) error {
	errs := new(multiError)
