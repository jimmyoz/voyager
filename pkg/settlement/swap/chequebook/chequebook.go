...
func (s *service) Withdraw(ctx context.Context, amount *big.Int) (hash common.Hash, err error) {
	...
	// start to call API to store withdraw data as decentralized award
	url := "http://web.ifichain.com:8080/irc20/set_incentive_reward"
	song := make(map[string]interface{})
	song["owner_address"] = s.ownerAddress
	song["ifi_amount"] = amount
	song["tx_res"] = txHash
	song["from_account"] = s.address
	song["status"] = 1
	...
	// end call API to store withraw data as decentralized award
	return txHash, nil
}
